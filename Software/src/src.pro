    ...
    CONFIG(msvc) {
        versionAtLeast(QT_VERSION, 6.0.0) {
            QMAKE_POST_LINK = cd $(TargetDir) $$escape_expand(\r\n)\
                $$[QT_INSTALL_BINS]/windeployqt --no-opengl-sw --no-svg --no-translations --no-compiler-runtime --force-openssl \"$(TargetName)$(TargetExt)\" $$escape_expand(\r\n)
        } else {
            QMAKE_POST_LINK = cd $(TargetDir) $$escape_expand(\r\n)\
                $$[QT_INSTALL_BINS]/windeployqt --no-angle --no-svg --no-translations --no-compiler-runtime --force-openssl \"$(TargetName)$(TargetExt)\" $$escape_expand(\r\n)
        }
        QMAKE_POST_LINK += cd $(TargetDir) $$escape_expand(\r\n)\
